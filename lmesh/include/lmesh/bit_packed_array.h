...
/**
 * BitPackedArray is a compact data structure for storing fixed-width unsigned integers.
 * Each integer occupies exactly `width` bits, and the integers are packed end-to-end
 * in a contiguous array of uint64_t. This allows for efficient storage and random access
 * of large arrays of integers with a known maximum bit width.
 *
...
class BitPackedArray
{
  public:
    ...
    BitPackedArray(uint32_t width, size_t count) : width_(width), count_(count)
    {
