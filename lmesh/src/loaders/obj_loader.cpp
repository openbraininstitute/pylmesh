...
bool OBJLoader::load(const std::string& filepath, QuantizedMesh& out_mesh)
{
    ...
    for (const char* p = begin; p < end;)
    {
        ...
        if (p[0] == 'v' && p[1] == ' ')
        {
            p += 2;
            skip_spaces(p);
            const float x = parse_float(p);
            skip_spaces(p);
            const float y = parse_float(p);
            skip_spaces(p);
